...
### 1. Fetch
Centralisation de sources hétérogènes : fichiers CSV, extractions Excel multi-onglets et requêtes de tables HTML (via pd.read_html) directement depuis le site de l'INSEE pour récupérer le niveau de vie médian et le taux de pauvreté.

### 2. Clean
* Suppression des en-têtes et lignes de commentaires spécifiques aux formats de publication de l'INSEE.
* Standardisation des identifiants territoriaux : application de la méthode str.zfill(2) pour harmoniser les codes départements (ex: le code 1 devient 01).
...
* Réorganisation logique des colonnes : Métadonnées géographiques, puis Variables Explicatives (X), et enfin Variables Expliquées (Y).
...
Les violences extrêmes s'avèrent hautement stochastiques à l'échelle départementale (R² = 0.182). Seul le chômage émerge comme signal prédictif significatif.
Équation estimée : homicides = -0.0063 + 0.0022 * taux_chomage
...
Équation estimée : ITU = 0.7967 + 0.8456 * ln(densite_population) + 0.3285 * part_non_diplomes
* Performance : R² = 0.615 (Modèle statistiquement robuste).
...
Équation estimée : Total_Vols = -17.4076 + 5.6564 * ln(densite_population) + 0.4666 * taux_pauvrete_60
* Performance : R² = 0.621.
* Enseignement : Le modèle décrit une mécanique d'opportunité spatiale pure. La délinquance d'appropriation se cristallise là où la précarité économique (+4.67 vols pour 10 points de pauvreté) rencontre la concentration de cibles potentiels engendrée par la densité urbaine.
...
L'organisation des fichiers du projet est structurée ainsi :
- Folder DATA_PROJET/ contenant la base finale consolidée (base_departements_complete_2020.csv), le référentiel géographique (departements-france.csv) ainsi que l'ensemble des fichiers sources bruts de l'INSEE et du SSMSI.
- Fichier Analyse_Econometrique_Violence.ipynb qui représente le notebook principal de production et de modélisation.
- Fichier README.md servant de support de présentation pour l'ensemble du projet.
